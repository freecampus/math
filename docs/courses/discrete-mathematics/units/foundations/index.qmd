...
- Check examples with Python, SymPy, and `fcmath`.
:::
...
Each lesson now includes a guided practice layer so you can move from reading a
concept to using it independently:
...
- **checkpoint exercises** that ask you to repeat the same reasoning without as
  much support;
- a browser-native checkpoint or guessing game before the final appendix;
- final visible `fcmath`, Python, and SymPy examples for computational checks.

When a discrete math problem feels unclear, write the relevant definition,
construct a tiny example, and test the definition against that example. This
habit is more reliable than trying to memorize many special cases.
...
Use `fcmath` for small truth tables.

```{python}
#| echo: true
from fcmath.discrete_math import implies, truth_table
