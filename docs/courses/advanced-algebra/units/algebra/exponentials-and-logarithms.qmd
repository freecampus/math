...
execute:
  echo: false
id: advanced-algebra-algebra-exponentials-and-logarithms
...
outcome_ids: [advanced-algebra-outcome-1, advanced-algebra-outcome-2, advanced-algebra-outcome-3, advanced-algebra-outcome-5]
---
...
```{python}
#| fig-alt: "Graphs comparing exponential growth, exponential decay, and quadratic growth."
...
Exponentials build repeated multiplication. Logarithms answer exponent
questions. Guess which inverse operation you need before solving.
...
The base is `1.08`, which is larger than `1`, so the model grows by `8%` each
time period. At `t = 3`:
...
::: {.practice-box}
**Checkpoint.** Interpret `A(t) = 1200(0.85)^t`. Is it growth or decay? What is
the percent change each time period?
:::
...
::: {.practice-box}
**Checkpoint.** Rewrite `log_3(81) = 4` in exponential form. Then evaluate
`log_5(1/25)`.
:::
...
::: {.practice-box}
**Checkpoint.** Solve `5^(2x) = 125` and `log_3(x - 1) = 2`. Include a domain
check for the logarithmic equation.
:::
...
Use parsed equations with the solver for exact symbolic answers, then interpret
them using exponent and logarithm rules.
