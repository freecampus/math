...
- verify computations with `fcmath`, Python, and SymPy.
:::
...
Use `fcmath` to inspect the trigonometry path.

```{python}
#| echo: true
from fcmath.trigonometry import TRIGONOMETRY_PATH

TRIGONOMETRY_PATH.slugs()
```

Use `fcmath` for basic angle helpers.

```{python}
#| echo: true
from fcmath.trigonometry import coterminal_angle, reference_angle_degrees
