...
from fcmath.core import infer_variable
from fcmath.settings import get_settings
...
def solve_equation_steps(
    ...
    *,
    ...
    :func:`fcmath.core.parse_equation` or :func:`fcmath.core.parse_expression`
    before calling this function when the starting point is user text.

    SymPy/fcmath produce the answer and algebraic transformations locally. If
    ``explain=True`` and an OpenAI key has been configured through
    :func:`fcmath.settings.configure`, an optional AI tutor explanation is
...
def _coerce_equation(equation: EquationInput) -> sp.Equality:
    if isinstance(equation, str):
        msg = (
            ...
            "not a string. Use fcmath.core.parse_equation() first."
        )
        raise TypeError(msg)
